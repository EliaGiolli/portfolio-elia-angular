...
- **`app-icon`**: resolves `name` to `assets/icons/{tech|miscellaneous}/{name}.svg` via `TECH_ICONS` in `shared/types/techMeta.ts`. Always set `decorative` explicitly for purely visual icons (emits `aria-hidden="true"` + `alt=""`); otherwise it falls back to `"{techLabel(name)} icon"` alt text if none is given. Two more inputs matter:
  - `size` is the size of the **glyph**. The circular chip is opt-in via `badge` and is drawn on the host, *around* that glyph — so `[size]="28" badge` is a 28px icon in a 48px chip. Don't put padding back on the image: with the global `box-sizing: border-box` it silently shrinks the glyph (it used to make `[size]="20"` render nothing at all).
  - `tone` picks the rendering technique. `brand` (default) emits an `<img>` and keeps the asset's own colours — right for Simple Icons marks and for `github`/`linkedin`. `tone="current"` emits a `mask-image` span painted with `currentColor`, which is the only way a Lucide stroke icon can follow the colour of the button or link it sits in. Every icon inside an `app-button` should be `tone="current"`, small (~18px) and unbadged.
